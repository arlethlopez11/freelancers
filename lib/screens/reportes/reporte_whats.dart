...
import 'package:freelancers/db/whats.dart';
...
class ReporteWhats extends StatefulWidget {
  
  @override
 _ReporteWhatsState createState() => _ReporteWhatsState();

}

class _ReporteWhatsState extends State<ReporteWhats>{
  

  WhatsService whatsService = WhatsService();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
    appBar: AppBarFreelancer(titulo: "Taxis Ixcamparic"),
  ...
  getCallsByRange(DateTime fechaIni, DateTime fechaFin) async{
      this.llamadaList =  await whatsService.getCallsByRange(fechaIni, fechaFin);
      List<DataColumn> columns = this.getEncabezado(); 
      List<DataRow> rows = this.getRows(this.llamadaList);
      changeScreen(context, PlantillaReporte("Reporte Whatsapp",columns,rows));
      
  }

List<DataColumn> getEncabezado() {
  List<String>  encabezado = ["Fecha","Piloto","# Cliente","Cliente"]; 
